...
# Display the first few rows of all four dataframes to verify the contents
print("NASA TLX Data:")
...
merged_data = pd.merge(merged_data, maze_results_data, how='left', on='Row Index')

# Drop the Row Index column
merged_data.drop(columns=['Row Index'], inplace=True)

# Rename columns for clarity (update as needed if there are similar column names)
# Example renaming (assuming there are columns to clarify):
